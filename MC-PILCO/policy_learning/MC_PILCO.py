...
class MC_PILCO_CNF(torch.nn.Module):
    """
    Monte-Carlo Probabilistic Inference for Learning Control with CNF
    """
    def __init__(self, T_sampling, state_dim, input_dim, f_sim, 
                 f_model_learning, model_learning_par, 
                # CHANGE: add flows learning params
                 f_flows_learning, flows_learning_par,
        ...
        # CHANGE: get the flows learning object
        print('\n\nGet the flows learning object...')
        self.flows_learning = f_flows_learning(**flows_learning_par)
    ...
    def reinforce(self, initial_state, initial_state_var,
        ...
        for trial_index in range(first_trial_index, last_trial_index):
            ...
            # TODO: test flows learning performance?
    ...
    def rollout(self, data_collection_index, T_rollout = None, particle_pred = False):
        ...
        for t in range(1,T_rollout):
            ...
            # MASON
            next_state, delta_mean, delta_var = self.model_learning.get_next_state(current_state = rollout_trj[t-1:t,:],
    ...
    def reinforce_policy(self, T_control, num_particles, trial_index, 
        ...
        with torch.no_grad():
            ...
            while num_attempts < 10  and flg_nan:
                ...
                states_sequence_NODROP, states_mean_sequence_NODROP, states_var_sequence_NODROP, inputs_sequence_NODROP = self.apply_policy(particles_initial_state_mean = particles_initial_state_mean,
                                                                                   particles_initial_state_var = particles_initial_state_var,
        ...
        while opt_step < num_opt_steps:
            ...
            while num_attempts < 10  and flg_nan:
                # apply the policy
                # input is ths state action
                states_sequence, states_mean_sequence, states_var_sequence, inputs_sequence = self.apply_policy(particles_initial_state_mean = particles_initial_state_mean,
                                                                     particles_initial_state_var = particles_initial_state_var,
                                                                     flg_particles_init_uniform = flg_particles_init_uniform,
                                                                     flg_particles_init_multi_gauss = flg_particles_init_multi_gauss,
                                                                     particles_init_up_bound = particles_init_up_bound,
    ...
    def apply_policy(self, particles_initial_state_mean,
        ...
        states_mean_sequence_list = []
        states_var_sequence_list = []

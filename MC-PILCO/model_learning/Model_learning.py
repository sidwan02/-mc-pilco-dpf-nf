...
class Model_learning(torch.nn.Module):
    ...
    def get_SOR_gp_estimate(self, gp_inputs, gp_index_list = None):
        ...
        for gp_index in gp_index_list:
            gp_output_mean_list[gp_index],\
            gp_output_var_list[gp_index] = self.gp_list[gp_index].get_SOR_estimate_from_alpha(X_test = gp_inputs,
    ...
    def get_L1_gp_estimate(self, gp_inputs, gp_index_list = None):
        ...
        for gp_index in gp_index_list:
            gp_output_mean_list[gp_index],\
            gp_output_var_list[gp_index] = self.gp_list[gp_index].get_estimate_from_alpha(X = self.gp_inputs_tr_list[gp_index][self.alpha_indices_L1_list[gp_index],:], 
                                                                                          X_test = gp_inputs,
    ...
    def train_gp_likelihood(self, gp_index, optimization_opt_dict):
        ...
        dataset = torch.utils.data.TensorDataset(self.gp_inputs, self.gp_output_list[gp_index]/self.norm_list[gp_index])
...
class Model_learning_RBF(Model_learning):
    ...
    def __init__(self, num_gp, init_dict_list,
                 approximation_mode = None, approximation_dict = None,
                 dtype = torch.float64, device = torch.device('cpu'), flg_norm = False):
